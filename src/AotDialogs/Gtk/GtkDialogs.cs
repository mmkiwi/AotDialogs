...
public class GtkDialogs : INativeDialog
{
    public static GtkDialogs TryLoad(string? gtkName = null, string? glibName = null)
    {
        ...
        if (NativeMethods.gtk_get_major_version() != 3)
        {
            throw new NotSupportedException("GTK version not supported");
        }
        
        return new GtkDialogs(NativeMethods.gtk_get_major_version(), NativeMethods.gtk_get_minor_version(), NativeMethods.gtk_get_micro_version());
        ...
        IntPtr ResolveGtk(string libraryName, Assembly assembly, DllImportSearchPath? searchPath)
        {
            if (libraryName == "gtk")
            {
                ...
                if (NativeLibrary.TryLoad("gtk", out gtkHandle)) return gtkHandle;
                if (NativeLibrary.TryLoad("libgtk", out gtkHandle)) return gtkHandle;
                if (NativeLibrary.TryLoad("libgtk-3.0", out gtkHandle)) return gtkHandle;
            }
            if (libraryName == "glib")
            {
